...
Screen::Screen() {	
	...
	ultrabigfont = TTF_OpenFont("res/fonts/dinprolight.otf", 140);
	...
	//wczytywanie info
	surface = IMG_Load("res/img2/info.png");
	img_info = SDL_GL_MakeTexture(surface);
	SDL_FreeSurface(surface);

	//wczytywanie final
	surface = IMG_Load("res/img2/final.png");
	img_final = SDL_GL_MakeTexture(surface);
	SDL_FreeSurface(surface);
	...
	SDL_FreeSurface(surface);
	surface = IMG_Load("res/img2/b_blue03.png");
	img_block0403 = SDL_GL_MakeTexture(surface);
	SDL_FreeSurface(surface);
	surface = IMG_Load("res/img2/b_blue02.png");
	img_block0402 = SDL_GL_MakeTexture(surface);
	SDL_FreeSurface(surface);
	surface = IMG_Load("res/img2/b_blue01.png");
	img_block0401 = SDL_GL_MakeTexture(surface);
	SDL_FreeSurface(surface);
	...
	pause_menu_texture[3] = SDL_GL_MakeTextTexture("controls", menufont, darkblue, pause_menu_texture_offset[3]);
	...
	finalscore_texture[0] = SDL_GL_MakeTextTexture("game over. your score:", titlefont, darkblue, finalscore_texture_offset[0]);
	...
	for (int i=0; i<10; i++) {
		char tmp[2] = {(char)(i+'0'),'\0'};
		final_number_texture[i] = SDL_GL_MakeTextTexture(tmp, ultrabigfont, white, final_number_texture_offset[i]);
	}	
}
...
void Screen::drawLevel(vbn bonuses, vbl blocks, Player *player, int blocksuntouched[]) {
	...
	for (vbl::iterator it = blocks.begin(); it != blocks.end(); it++) {
		...
		else if((*it).get_type()==4) {
			if((*it).get_lives()==1) SDL_GL_RenderTexture(img_block0401, &location);
			else if((*it).get_lives()==2) SDL_GL_RenderTexture(img_block0402, &location);
			else if((*it).get_lives()==3) SDL_GL_RenderTexture(img_block0403, &location);
		}
	}
...
void Screen::drawFinalScore(int prscore) {
	...
	SDL_GL_RenderTexture(img_final, &location);
	...
	//renderowanie "gameover"
	finalscore_texture_offset[0].x = 65; finalscore_texture_offset[0].y = 190;
	SDL_GL_RenderTexture(finalscore_texture[0], &finalscore_texture_offset[0]);
	
	for (int i=0; i<8; i++) {
		int k = (int)(printscore[i]-'0');
		final_number_texture_offset[k].x = 64+i*70; final_number_texture_offset[k].y = 220;
		SDL_GL_RenderTexture(final_number_texture[k], &final_number_texture_offset[k]);
	}
...
void Screen::drawInfo() {
	...
	SDL_GL_RenderTexture(img_info, &location);
