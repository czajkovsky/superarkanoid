...
Player::Player(const int &lives, const int &level, const int &ulx, const int &uly, const int &brx, const int &bry) {
	...
	this->platform = new Platform(ulx, uly, brx, bry);
}
...
void Player::addLives(const int &k) {
	this->lives = min(this->lives+k, 99);
}
