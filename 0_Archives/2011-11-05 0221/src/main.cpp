...
*/
	...
	SDL_Surface *source = IMG_Load("kolo.png");
	SDL_Surface *kolo = SDL_DisplayFormatAlpha(source);

	source = IMG_Load("czarny.jpg");
	SDL_Surface *black = SDL_DisplayFormat(source);
...
//		cout<<running<<"\n";
		...
		for (vbo::iterator it = borders.begin(); it != borders.end(); it++) {
			...
			SDL_BlitSurface(black, &clip, screen, &offset);
		}
		...
		for (vba::iterator it = balls.begin(); it != balls.end(); it++) {
				...
				SDL_BlitSurface(kolo, NULL, screen, &offset);
		}
